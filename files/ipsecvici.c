...
#include <libvici.h>
#include "ipsecvici.h"
#include <na_encrypt.h>

#define INITIAL_RESPONSE_SIZE 4096 // 4KB

int get_ip(char *intf);
int is_ipaddr(char *str);

/**
 * @brief      Appends given msg to the the response string
 *
 * @param      response   The response to be appended
 * @param      offset     The offset
 * @param      resp_size  The response size
            ...
            size_t *resp_size, const char *format, ...)
{
    ...
    if (*offset + needed > *resp_size) {
        fprintf(stderr, "IPsec: response buffer overflow (needed %d, available %zu)\n", needed, *resp_size - *offset);
        va_end(args);
        return -1;
    }
...
/**
 * @brief      sa list type values like traffic selectors
 *
 * @param      user   The user
 * @param      res    The resource
 * @param      name   The name
...
/**
 * @brief      ike sas gives ike connection name
 *
 * @param      user  The user
 * @param      res   The resource
 * @param      name  The name
...
/**
 * @brief      call back function registered for list-sa
 *
 * @param      user  The user
 * @param      name  The name
 * @param      res   The resource
...
/**
 * @brief      lists ike sa details equivalent to
 * "swanctl --list-sa --ike <conn> --noblock"
 *
 * @param      conn       The connection
 * @param[in]  ike_conn   The ike connection struct
...
/**
 * @brief      terminates the given ike connection equivalent
 * to "swanctl -t --ike <conn>"
 *
 * @param      conn       The connection
 * @param[in]  ike_conn   The ike connection struct
 * @param      response   The response
...
/**
 * @brief      unloads the given ike connection
 *
 * @param      conn       The ike connection
 * @param[in]  ike_conn   The ike connection struct
 * @param      response   The response
 * @param      offset     The offset
 * @param      resp_size  The response size
...
/**
 * @brief      loads a given ike connection
 *
 * @param      conn       The connection
 * @param[in]  ike_conn   The ike connection struct
 * @param      response   The response
            ...
            char **response, size_t *offset, size_t *resp_size)
{
    ...
    vici_add_key_valuef(req, "dpd_timeout", "%s", ike_conn.dpd_timeout);
    ...
    ret = decrypt_txt((unsigned char *)ike_conn.psk,&psk,&psk_len);
    if(ret<0) {
        append_res(response, offset, resp_size,
            "IPsec: psk decryption failed.");
        return -1;
    }
    vici_add_key_value(req, "data", psk, psk_len);
...
/**
 * @brief      Determines whether the specified string is ipaddr or not.
 *
 * @param      str   The ipaddr string
 *
 * @return     True if the specified string is ipaddr, False otherwise.
 */
int is_ipaddr(char *str)
{
...
/**
 * @brief      parse netlink response
 *
 * @param      nlh         The nlh
 * @param      ifname      The ifname
 * @param[in]  default_rt  The default right
 * @param      sockfd_ptr  The sockfd pointer
 *
 * @return     0/1 on success/failure
 */
...
int get_ip(char *intf)
{
    ...
    fprintf(stderr, "1intf=%s\n", intf);
    if (strstr(intf, "pppoe")) default_rt = 2;
    else if (strstr(intf, "ETH")) {
    ...
    else if (strstr(intf, "usb") && strstr(intf, "v6")) {
        intf[strlen(intf)-2]=0;
    }
    else if (strstr(intf, "usb")) ;
    else if (strstr(intf, "lan") && strstr(intf, "v6")) {
        intf[strlen(intf)-2]=0;
...
/**
 * @brief      determines given ip is within the given subnet range
 *
 * @param      ip         The ipaddr
...
/**
 * @brief      Reads a configuration from uci file
 *
 * @param      ike_conn   The ike connection struct
 * @param[in]  ike        The ike connection name
 * @param      response   The response
 * @param      offset     The offset
 * @param      resp_size  The response size
 *
        ...
        char **response, size_t *offset, size_t *resp_size)
{
    char buff[512] = {'\0'},
         ...
         is_pppoe[IP_LEN] = {'\0'},
         ike_version[16] = {'\0'};
    char local_acl[MAX_SUBNETS*MAX_SUBNETS][IP_LEN] = {{'\0'}},
         remote_acl[MAX_SUBNETS*MAX_SUBNETS][IP_LEN] = {{'\0'}};
    int cnt_l = 0,
        cnt_r = 0,
        acl_cnt1 = 0,
        acl_cnt2 = 0,
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        ...
        if (!strncmp(buff, "\toption ", 8)) {
            ...
            else if (!strncmp(key, "dpdtimeout", 10)) strncpy(ike_conn->dpd_timeout, value, 8);
        ...
        if (!strncmp(buff, "\tlist ", 6)) {
            char key[IP_LEN], value[IP_LEN];
            sscanf(buff, "\tlist %16s '%40[^']'", key, value);
    ...
    if ( !strncmp(local_gateway, "Any", 3) && strstr(ike_conn->remote_addrs, ":") ) {
        strncat(local_gateway, "v6", 3);
    }
    strncpy(is_pppoe, local_gateway, IP_LEN);
    ...
    if (!is_ipaddr(local_gateway)) {
        append_res(response, offset, resp_size,
            "IPsec: '%s' isn't an ip address. check for PPPoE\n", local_gateway);
        char pppoe_intf[IP_LEN+8];
        snprintf(pppoe_intf, IP_LEN+8, "pppoe-%s", is_pppoe);
        if(-1 == get_ip(pppoe_intf)) {
            append_res(response, offset, resp_size,
            "IPsec: can't get the ipaddr of '%s': %s\n", pppoe_intf, strerror(errno));
            return -1;
        }
        if (!is_ipaddr(pppoe_intf)) {
            append_res(response, offset, resp_size,
            "IPsec: '%s' isn't an ip address.\n", pppoe_intf);
            return -1;
        }
        strncpy(local_gateway, pppoe_intf, IP_LEN);
    }

    strncpy(ike_conn->local_addrs, local_gateway, strlen(local_gateway));
    if (!strlen(ike_conn->local_id))
        strncpy(ike_conn->local_id, local_gateway, strlen(local_gateway));
    ...
    if(peermode == 1) {
        ...
        ike_conn->child_cnt = (cnt_l)*(cnt_r);
        for (int k=0; k<ike_conn->child_cnt; ) {
            for (int i=0; i<ike_conn->cnt_l; ++i) {
                for (int j=0; j<ike_conn->cnt_r; ++j, ++k) {
                    ike_conn->child_sa[k].left = i;
                    ike_conn->child_sa[k].right = j;
                    snprintf(ike_conn->child_sa[k].name, INSTANCE_LEN+16, "%s_child%u", ike_conn->name, k+1);
                }
            }
        }
    }
...
/**
 * @brief      connects to charon daemon and implements the given action
 *
...
{
    static char buffer[INITIAL_RESPONSE_SIZE] = {'\0'};
    ...
    static IKE_CONN_T ike_conn = {0};
    memset(&ike_conn, 0, sizeof(IKE_CONN_T));
    ...
    if (conn) {
        ...
        vici_disconnect(conn); // Disconnect from charon
    } else {
        append_res(&response, &offset, &resp_size,
